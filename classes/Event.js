...
export class Event {
  ...
  setValue(value, path, store = false, cb = false) {
    ...
    if (path_arr) {
      ...
      if (obj) {
        ...
        if (store) {
          database().ref("clubs/" + this.data.club_id + "/events/" + this.data.id + '/' + path).set(value);
        } else 
  ...
  getDatabaseValue(path, cb) {
    database().ref("clubs/" + this.data.club_id + "/events/" + this.data.id + '/' + path).once("value", function(snap) {
  ...
  onClickBell() {
    ...
    user.toggleEventNotification(this.getClubID(), this.getID(), function(subscribed) {
      if (subscribed) 
        this.data.utils.showToast('Benachrichtigung aktiviert', faBell);
      else 
        this.data.utils.showToast('Benachrichtigung deaktiviert', faBellSlash);
      if (this.renderListerner) 
        this.renderListerner(this);
      }
    .bind(this))
