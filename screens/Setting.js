...
export default class Setting extends React.Component {
  ...
  constructor(props) {
    ...
    var hasFlatList = this.props.navigation.getParam("hasFlatList", null);
    this.state = {
      headline: headline,
      club: club,
      utils: utils,
      ...
      childs: childs,
      hasFlatList: hasFlatList
    };

    if (!this.state.club) 
      this.state.club = {};
    if (!this.state.club.invite_codes) 
      this.state.club.invite_codes = {};
    
    //firebase listener
    if (this.state.type == "qrcodes") {
      database().ref("clubs/" + club.id + "/invite_codes").on("value", function(snap) {
        if (snap.val()) 
          this.state.club.invite_codes = snap.val();
        else 
          this.state.club.invite_codes = {};
        this.forceUpdate();
      }.bind(this));
  ...
  _openGroupOptions(g_key) {
    ActionSheetIOS.showActionSheetWithOptions({
      ...
      if (buttonIndex === 0) {
        ...
        //delete this groups
        this.state.utils.showAlert("Gruppe löschen?", "", [
          "Ja", "Nein"
        ], function(btn_id) {
          if (btn_id == 0) {
            // delete group
    ...
    ActionSheetIOS.showActionSheetWithOptions({
      ...
      if (buttonIndex === 0) {
        ...
        RNFetchBlob.config({
          ...
          fileCache: true,
          appendExt: "image"
        }).fetch("GET", this.state.club.invite_codes[code].img, {"Cache-Control": "no-store"}).progress({
          count: 1000
        }, (received, total) => {
        ...
        //delete qrcode
        this.state.utils.showAlert("QR-Code löschen?", "", [
          "Ja", "Nein"
        ], function(btn_id) {
          if (btn_id == 0) {
            // delete code
  ...
  _editGroup() {
    ...
    if (key != "NEW") {
      ...
      database().ref("clubs/" + this.state.club.id + "/groups/" + key + "/has_admin_rights").set(group.has_admin_rights);
      this.forceUpdate();
    } else {
      group.key = null;
  ...
  _editInviteCode() {
    ...
    Object.keys(invite.groups).map(code => {
      if (invite.groups[code] === true) 
        filteredGroups[code] = true;
      }
    );
  ...
  _updateClubName(name) {
    if (this.state.callback) {
      database().ref("clubs/" + this.state.club.id + "/name").once("value", function(snap) {
        database().ref("clubs/" + this.state.club.id + "/name").set(name);
        if (snap.val() != name) {
  ...
  render() {
    ...
    var pageContent = <Theme.Text>Setting not found</Theme.Text>;
    ...
    if (this.state.type == "groups") {
      ...
      if (club.invite_codes) {
        ...
        if (curCode) {
          groupsList = Object.keys(club.groups).map(g_key => {
            ...
            var iconColor = group_active
              ? "white"
              : "#ADA4A9";
            var icon = group_active
              ? faTimesCircle
              : faPlusCircle;
            ...
            return (
              <Theme.TouchableOpacity
                onPress={() => {
                  club.invite_codes[this.state.modal.invite.code].groups[g_key] = !club.invite_codes[this.state.modal.invite.code].groups[g_key];
                  this.forceUpdate();
                }}
                key={g_key}
                style={{
      ...
      pageContent = Object.keys(this.state.colors).map(hex => {
        ...
        return (
          <Theme.TouchableOpacity
            ...
            {
              color.selected
                ? (<FontAwesomeIcon size={22} color={"#" + color.font_hex} icon={faCheck}/>)
                : (void 0)
            }
          </Theme.TouchableOpacity>
        );
      ...
      pageContent = (
        <Theme.View>
          <InputBox marginTop={-15} value={club.name} onChange={text => (this.state.club.name = text)}/>
          <Button marginTop={40} label="Fertig" onPress={() => this._updateClubName(this.state.club.name)}/>
        </Theme.View>
      );
    }
          ...
          <Theme.View>
            ...
            <Theme.View
              ...
              <Switch
                ...
                thumbColor={this.state.modal.group.public
                  ? "#1e1e1e"
                  : "#f4f3f4"
}
            ...
            <Theme.View
              ...
              <Switch
                ...
                thumbColor={this.state.modal.group.has_admin_rights
                  ? "#1e1e1e"
                  : "#f4f3f4"
}
                ios_backgroundColor="#3e3e3e"
