...
export default class Setting extends React.Component {
	...
	constructor(props) {
		...
		if (this.state.type == 'qrcodes') {
			database().ref('clubs/' + club.id + '/invite_codes').on(
				'value',
				(function(snap) {
		...
		ActionSheetIOS.showActionSheetWithOptions(
			...
			buttonIndex => {
				if (buttonIndex === 0) {
					...
					this.state.modal.group = JSON.parse(JSON.stringify(this.state.club.groups[g_key]));
					this.state.modal.group.key = g_key;
					if (!this.state.modal.group.has_admin_rights) this.state.modal.group.has_admin_rights = false;
					if (this.state.modal.group.public !== false) this.state.modal.group.public = true;
					...
					this.state.utils.showAlert(
						...
						(function(btn_id) {
							if (btn_id == 0) {
								...
								database().ref('clubs/' + this.state.club.id + '/groups/' + g_key).remove();
								this.forceUpdate();
							}
						}).bind(this),
						true,
		...
		ActionSheetIOS.showActionSheetWithOptions(
			...
			buttonIndex => {
				if (buttonIndex === 0) {
					...
					this.state.modal.invite = JSON.parse(JSON.stringify(this.state.club.invite_codes[code]));
					...
					RNFetchBlob
						...
						.then(res => {
							...
							FileViewer.open(Platform.OS === 'android' ? 'file://' + res.path() : '' + res.path())
								.then(() => {})
								.catch(error => {});
						});
					...
					this.state.utils.showAlert(
						...
						(function(btn_id) {
							if (btn_id == 0) {
								...
								database().ref('clubs/' + this.state.club.id + '/invite_codes/' + code).remove();
								this.forceUpdate();
							}
						}).bind(this),
						true,
	...
	_editGroup() {
		...
		if (key != 'NEW') {
			...
			database().ref('clubs/' + this.state.club.id + '/groups/' + key + '/name').set(group.name);
			database().ref('clubs/' + this.state.club.id + '/groups/' + key + '/public').set(group.public);
			database()
				.ref('clubs/' + this.state.club.id + '/groups/' + key + '/has_admin_rights')
				.set(group.has_admin_rights);
			this.forceUpdate();
		} else {
			group.key = null;
			key = database().ref('clubs/' + this.state.club.id + '/groups').push(group).key;
			this.state.club.groups[key] = group;
			this.forceUpdate();
		}
	}
	...
	_editInviteCode() {
		...
		var filteredGroups = {};
		var invite = JSON.parse(JSON.stringify(this.state.club.invite_codes[this.state.modal.invite.code]));
		Object.keys(invite.groups).map(code => {
			if (invite.groups[code] === true) filteredGroups[code] = true;
		});
		invite.groups = filteredGroups;
		this.state.club.invite_codes[this.state.modal.invite.code] = JSON.parse(JSON.stringify(invite));
	...
	render() {
		...
		if (this.state.type == 'groups') {
				...
				if (group) {
					return (
						...
						>
							<TouchableOpacity
								style={{
									backgroundColor: group.has_admin_rights ? '#16FFD7' : '#38304C',
									padding: 7,
									borderRadius: 15,
									borderBottomLeftRadius: group.public && group.has_admin_rights ? 0 : 15,
									borderBottomRightRadius: group.public && group.has_admin_rights ? 0 : 15,
									flexWrap: 'wrap',
							...
							>
								...
								<TouchableOpacity style={{ padding: 8, opacity: 0.7 }} onPress={() => this._openGroupOptions(key)}>
									<FontAwesomeIcon size={22} color={group.has_admin_rights ? '#38304C' : 'white'} icon={faEllipsisV} />
								</TouchableOpacity>
							</TouchableOpacity>
							...
							{group.public && group.has_admin_rights
								? <View
									...
									>
										<FontAwesomeIcon size={15} color="white" icon={faExclamationCircle} />
										<Text style={{ marginLeft: 10, color: 'white', fontSize: 16, fontFamily: 'Poppins-Bold' }}>
							...
							>
								<TouchableOpacity
									style={{
										...
										padding: 7,
										borderRadius: 15,
										borderBottomLeftRadius: invite.qr_code_ready === false ? 0 : 15,
										borderBottomRightRadius: invite.qr_code_ready === false ? 0 : 15,
										flexWrap: 'wrap',
										alignItems: 'flex-start',
										flexDirection: 'row',
									}}
								...
								>
									<View style={{ padding: 8 }}>
										<FontAwesomeIcon size={24} color="#16FFD7" icon={faQrcode} />
									</View>
									<View
										style={{
											marginLeft: 15,
									...
									>
										...
										>
											...
											{' '}
											{Object.keys(invite.groups).length == 1 ? 'Gruppe' : 'Gruppen'}
										</Text>
									</View>
									<TouchableOpacity style={{ padding: 8, opacity: 0.7 }} onPress={() => this._openQrcodeOptions(code)}>
										<FontAwesomeIcon size={22} color={'white'} icon={faEllipsisV} />
									</TouchableOpacity>
								</TouchableOpacity>
								...
								{invite.qr_code_ready === false
									? <View
										...
										>
											<ActivityIndicator style={{ transform: [ { scale: 0.9 } ] }} size="small" color="#38304C" />
											<Text
												style={{
													marginLeft: 10,
													color: '#38304C',
				...
				if (curCode) {
					groupsList = Object.keys(club.groups).map(g_key => {
						...
						return (
							<TouchableOpacity
								onPress={() => {
									club.invite_codes[this.state.modal.invite.code].groups[g_key] = !club.invite_codes[
				...
				return (
					<TouchableOpacity
						...
						onPress={() => {
							...
							database().ref('clubs/' + club.id + '/color').set(hex);
							this.forceUpdate();
						}}
					>
			...
			pageContent = (
				<View style={{ flexWrap: 'wrap', alignItems: 'flex-start', flexDirection: 'row' }}>{pageContent}</View>
			);
		}
					...
					<View>
						...
						>
							...
							<Switch
								...
								thumbColor={this.state.modal.group.public ? '#38304C' : '#f4f3f4'}
						...
						>
							...
							<Switch
								...
								trackColor={{ false: '#575757', true: '#16FFD7' }}
								thumbColor={this.state.modal.group.has_admin_rights ? '#38304C' : '#f4f3f4'}
								ios_backgroundColor="#3e3e3e"
								onValueChange={() => {
